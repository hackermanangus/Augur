...
pub enum AugurError {
    ...
    // No novels to update
    NonExistentNovel,
    // Novel doesn't exist in db
    NonExistentGuild,
    // Guild doesn't exist in db
    UniqueConstraint,
    // Novel already setup in channel
    NoChapters,
    // No chapters found for novel
    InvalidLink,
    // Link provided for novel is invalid
    FailedQuery,
    // Query to db has failed
...
impl Display for AugurError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match &self {
            ...
            AugurError::EmptyDatabase => { write!(f, "`{:?}`: Database is empty", self) }
        }
    }
}
